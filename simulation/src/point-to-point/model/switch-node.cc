...
void SwitchNode::PrintSwitchQlen(FILE* qlen_output){
	...
	for(uint32_t i = 1; i < n_dev; ++i){
		...
		for(uint32_t j=0; j < qCnt; ++j){
			fprintf(qlen_output, "%llu, %u, %u, %u, %llu, %llu\n", (unsigned long long)Simulator::Now().GetTimeStep(), m_id, i, j, (unsigned long long)m_mmu->egress_bytes[i][j], (unsigned long long)port_len);
			fflush(qlen_output);
		}
...
void SwitchNode::PrintSwitchBw(FILE* bw_output, uint32_t bw_mon_interval){
	...
	for(uint32_t i = 1; i < n_dev; ++i){
		...
		bw = bw*1.0 / 1e9; // Gbps
		fprintf(bw_output, "%llu, %u, %u, %f\n", (unsigned long long)Simulator::Now().GetTimeStep(), m_id, i, bw);
		fflush(bw_output);
