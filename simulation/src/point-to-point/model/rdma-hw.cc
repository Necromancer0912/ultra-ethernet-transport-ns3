...
namespace ns3
{
	...
	{
		m_uetIntegration = CreateObject<RdmaHwUetIntegration>();
		uint32_t numNodes = NodeList::GetNNodes();
		uint64_t linkBandwidth = 100000000000ULL; // 100Gbps fallback
		if (!m_nic.empty() && m_nic[0].dev != NULL) {
			linkBandwidth = m_nic[0].dev->GetDataRate().GetBitRate();
		}
		m_uetIntegration->Initialize(numNodes, linkBandwidth, m_mtu);
	...
	int RdmaHw::ReceiveUdp(Ptr<Packet> p, CustomHeader &ch)
	{
		...
		// UET integration hook (RX path)
		if (m_uetIntegration) {
			uint32_t srcId = (ch.sip >> 8) & 0xffff;
			uint32_t dstId = (ch.dip >> 8) & 0xffff;
			bool generatePdsResponse = m_uetIntegration->ProcessRxPacket(p, srcId, dstId, ch.udp.sport, ch.udp.pg, 0 /* UetPdsHeader::RUD */, 0.0);
			if (!generatePdsResponse) {
				return 0; // Simulated ACK loss, or rod mode out-of-order wait
			}
		}
		...
		{ // generate ACK or NACK
			...
			// UET Default Response Hook (Completion Path)
			if (m_uetIntegration) {
				uint32_t srcId = (ch.sip >> 8) & 0xffff;
				uint32_t dstId = (ch.dip >> 8) & 0xffff;
				m_uetIntegration->GenerateDefaultResponse(srcId, dstId, ch.udp.sport, ch.udp.pg, 0 /* UetPdsHeader::RUD */);
				// (In full stack, we would append standard response code to newp, but we just trigger hook for now)
			}
	...
	Ptr<Packet> RdmaHw::GetNxtPacket(Ptr<RdmaQueuePair> qp)
	{
		...
		// UET integration hook (TX path)
		if (m_uetIntegration) {
			uint32_t srcId = m_node ? m_node->GetId() : 0;
			uint32_t dstId = (qp->dip.Get() >> 8) & 0xffff; // Extract destination node ID from IP
			p = m_uetIntegration->ProcessTxPacket(p, srcId, dstId, qp->sport, qp->m_pg, 0 /* UetPdsHeader::RUD */, 0.0);
			if (!p) {
				return nullptr; // Packet dropped at UET engine
			}
		}

		// add RDMA sequence/timestamp header
	...
	void RdmaHw::PrintHostBW(FILE *bw_output, uint32_t bw_mon_interval)
	{
		for (int i = 0; i < m_nic.size(); ++i)
		{
			...
			bw = bw * 1.0 / 1e9;														// Gbps
			fprintf(bw_output, "%llu, %u, %u, %f\n", (unsigned long long)Simulator::Now().GetTimeStep(), m_node->GetId(), i, bw);
			fflush(bw_output);
	...
	void RdmaHw::PrintQPRate(FILE *rate_output)
	{
		...
		for (; it != m_qpMap.end(); it++)
		{
			...
			fprintf(rate_output, "%llu, %u, %u, %u, %u, %llu, %llu\n", (unsigned long long)Simulator::Now().GetTimeStep(), qp->m_src, qp->m_dest, qp->sport, qp->dport, (unsigned long long)qp->m_size, (unsigned long long)qp->m_rate.GetBitRate());
			fflush(rate_output);
	...
	void RdmaHw::PrintQPCnpNumber(FILE *cnp_output)
	{
		...
		for (; it != m_qpMap.end(); it++)
		{
			...
			{
				fprintf(cnp_output, "%llu, %u, %u, %u, %u, %llu, %llu\n", (unsigned long long)Simulator::Now().GetTimeStep(), qp->m_src, qp->m_dest, qp->sport, qp->dport, (unsigned long long)qp->m_size, (unsigned long long)qp_cnp[key]);
				fflush(cnp_output);
